#!/usr/bin/env python3
"""
Incremental MySQL backup and cleanup script with batched deletion and full DB dump.
"""

import datetime as dt
import gzip
import logging
import shutil
import subprocess
import sys
import time
from pathlib import Path
from typing import Dict

import mysql.connector

# ────────────────────── Configuration ──────────────────────
DB_HOST = "10.0.7.212"
DB_USER = "pg_user"
DB_PASS = "Dst0ao88lfxHnkHLkQd5Hf"
DB_NAME = "pgdb"

BACKUP_DIR = Path.home() / "backup"
FINAL_DIR = Path.home() / "glidex_backup"

TABLES = [
    "ismart_create_order",
    "customer_input_details",
    "transaction_details",
    "merchant_request4customer",
    "status_change_logs_model",
    "merchant_balance_sheet",
    "user_details",
    "callbacklog",
    "pginput_from_customer_details",
    "transaction_details_log",
]

RETENTION: Dict[str, int] = {
    "transaction_details_log": 1,
    "user_details": 60,
    "transaction_details": 60,
}

DEFAULT_RETENTION_DAYS = 7
BATCH_SIZE = 1000
SLEEP_BETWEEN_BATCHES = 0.5  # seconds

# ────────────────────── Logging ──────────────────────
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
log = logging.getLogger(__name__)


# ────────────────────── Helpers ──────────────────────
def timetag() -> str:
    return dt.datetime.now().strftime("%Y%m%d%H%M%S")


def run_cmd(argv, stdout=None) -> None:
    """Run a shell command with error handling."""
    try:
        subprocess.run(argv, check=True, stdout=stdout, stderr=subprocess.PIPE)
    except subprocess.CalledProcessError as e:
        log.error("Command failed: %s", e.stderr.decode())
        raise


def dump_table(table: str, days: int, out_file: Path) -> None:
    where = f"created < DATE_SUB(NOW(), INTERVAL {days} DAY)"
    cmd = [
        "mysqldump",
        "-h", DB_HOST,
        "-u", DB_USER,
        f"-p{DB_PASS}",
        DB_NAME,
        table,
        f"--where={where}",
    ]
    with gzip.open(out_file, "wb") as f:
        run_cmd(cmd, stdout=f)


def dump_full_db(out_file: Path) -> None:
    cmd = [
        "mysqldump",
        "-h", DB_HOST,
        "-u", DB_USER,
        f"-p{DB_PASS}",
        DB_NAME,
    ]
    with gzip.open(out_file, "wb") as f:
        run_cmd(cmd, stdout=f)


def purge_old_rows(table: str, days: int, conn) -> None:
    """Delete old rows in batches of 1000."""
    cursor = conn.cursor()
    total_deleted = 0
    while True:
        cursor.execute(
            f"DELETE FROM {table} WHERE created < DATE_SUB(NOW(), INTERVAL {days} DAY) LIMIT {BATCH_SIZE}"
        )
        deleted = cursor.rowcount
        conn.commit()
        if deleted == 0:
            break
        total_deleted += deleted
        log.info("   • Deleted %d rows from %s (total so far: %d)", deleted, table, total_deleted)
        time.sleep(SLEEP_BETWEEN_BATCHES)
    log.info("   ✓ Finished deleting from %s (%d rows total)", table, total_deleted)


# ────────────────────── Main Workflow ──────────────────────
def main():
    BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    FINAL_DIR.mkdir(parents=True, exist_ok=True)
    stamp = timetag()

    # 1. Full DB Backup
    full_dump_file = BACKUP_DIR / f"{DB_NAME}_full_{stamp}.sql.gz"
    try:
        log.info("📦 Creating full DB backup → %s", full_dump_file.name)
        dump_full_db(full_dump_file)
        log.info("✅ Full DB backup completed")
    except Exception as e:
        log.error("❌ Full DB backup failed: %s", e)
        sys.exit(1)

    # 2. Per-table dump + delete
    try:
        conn = mysql.connector.connect(
            host=DB_HOST,
            user=DB_USER,
            password=DB_PASS,
            database=DB_NAME,
            autocommit=False
        )
    except Exception as e:
        log.error("❌ Could not connect to MySQL: %s", e)
        sys.exit(1)

    for table in TABLES:
        retention_days = RETENTION.get(table, DEFAULT_RETENTION_DAYS)
        backup_file = BACKUP_DIR / f"{table}_{stamp}.sql.gz"

        try:
            log.info("📤 Dumping table '%s' older than %d days → %s", table, retention_days, backup_file.name)
            dump_table(table, retention_days, backup_file)
            log.info("✅ Dump complete for table '%s'", table)
        except Exception as e:
            log.error("❌ Dump failed for table '%s': %s", table, e)
            continue

        try:
            log.info("🧹 Purging old rows from '%s'…", table)
            purge_old_rows(table, retention_days, conn)
        except Exception as e:
            log.error("❌ Purge failed for table '%s': %s", table, e)
            continue

    conn.close()

    # 3. Move backup files
    try:
        for file in BACKUP_DIR.iterdir():
            shutil.move(str(file), FINAL_DIR / file.name)
        log.info("📁 All backup files moved to %s", FINAL_DIR)
    except Exception as e:
        log.error("❌ Failed to move backup files: %s", e)
        sys.exit(1)

    log.info("🏁 All backup and cleanup operations completed successfully.")


if __name__ == "__main__":
    main()
